Module Codigo
        ...
        For temporal = 1 To 300
            ...
            For Each vehiculo In conjuntoVehiculos
                ...
                If (strStart = "TRUE") Then
                    ...
                    moverVehiculo(nextStreet, vehiculo, 20, lista)
                End If
    ...
    Public Sub GirarVehiculoSegunOrientacionDeLaCalle(nextStreet As AcadEntity, vehiculo As AcadEntity, lista As ListBox)
        ...
        End If
        ''---------------------------------------
        ''Casteando acadentity a acadline para obtener .angle.
        'Dim l As AcadLine
        'l = nextStreet
        'l.Angle
        'nextStreet.angle
        ...
        'ca = Math.Abs((p2(0) - p1(0)))
        'co = Math.Abs((p2(1) - p1(1)))
        ...
        'MsgBox("Comparacion anglelinea= " & linea.Angle & "angle= " & angle)



        ''segundo cuadrante 
        'If pendiente < 0 And p2(1) > p1(1) Then
        '    angle = Math.PI - angle
        'End If
        ''cuarto cuadrante
        'If pendiente < 0 And p2(1) < p1(1) Then
        ...
        'MsgBox("linea.angle= " & linea.Angle & "angledegrees= " & angledegrees)
        ...
        If valor <> "VACIO" Then
            ...
            vehiculo.Rotate(p1, convertRadtoAngd(CDbl(valor)) * -1)
        End If
    ...
    Public Function moverVehiculo(calle As AcadEntity, vehiculo As AcadEntity, numMov As Integer, lista As ListBox)
        ...
        prueba = AnalizandoEntornoCircular(vehiculo)
        If prueba > 0 Then
            MsgBox("hay " & prueba & " ELEMENTOS DETECTADOS")
        End If
    ...
    Public Function changeTraficLights(nowtime As String, conjuntoSemaforos As AcadSelectionSet, lista As ListBox)
        ...
        segundos = CInt(Left(Right(nowtime, 5), 2))
        ...
        For Each semaforo In conjuntoSemaforos
            ...
            If (segundos > 10 And segundos <= 20) Or (segundos > 30 And segundos <= 40) Or (segundos > 50 And segundos <= 60) Then
                For i = 1 To numerocalles
                    handle = CInt(GetDictionarySemaforo(semaforo, CStr(i), valor))
                    If changer = True Then
                        changer = False
                        UpdateDictionaryToSemaforoTextField(semaforo, CStr(handle), "VERDE")
                    Else
                        changer = True
                        UpdateDictionaryToSemaforoTextField(semaforo, CStr(handle), "ROJO")
                    End If
                    lista.Items.Add("handler = " & handle & "Semaoforo = " & GetDictionarySemaforo(semaforo, handle, valor)) '
                Next
    ...
    Public Function addDictionaryToTrafficLights(conjuntoCalles As AcadSelectionSet, conjuntoSemaforos As AcadSelectionSet)
        ...
        For Each semaforo In conjuntoSemaforos
            ...
            contadorcalles = 0
            For Each calle In conjuntoCalles
                If semaforo.center(0) = calle.endpoint(0) And semaforo.center(1) = calle.endpoint(1) Then
                    contadorcalles = contadorcalles + 1
                    addXdata(semaforo, CStr(contadorcalles), CStr(calle.handle))
                    addXdata(semaforo, CStr(calle.handle), "STOPPED")
                    addXdata(semaforo, "CALLESSEMAFORO", CStr(contadorcalles))
                End If
            Next
        Next
    End Function
    ...
    Public Function shouldCarMove(vehiculo As AcadEntity)
    ...
    Public Function AnalizandoEntornoCircular(vehiculo As AcadEntity) As Integer
        ...
        'Dim nextPosition(0 To 2) As Double = Nothing
        Dim x, y, z, w, v, u As Double
        Dim valor As String
        Dim beta As Double
        Dim centroide() As Double
        Dim angulo As Double
        ...
        End If
        'If angle < 0 Then
        '    MsgBox("90-180.angulo= " & angulo & " centroide(x)= " & centroide(0) & " centroide(y)= " & centroide(1) & " x= " & x & "y= " & y)
        '    p(0) = centroide(0) + 2 * x
        '    p(1) = centroide(1) + 2 * y

        'End If
        'If angle >= 180 And angle < 270 Then
        '    MsgBox("180-270.angulo= " & angulo & " centroide(x)= " & centroide(0) & " centroide(y)= " & centroide(1) & " x= " & x & "y= " & y)
        '    p(0) = centroide(0) - 2 * x
        '    p(1) = centroide(1) - 2 * y

        'End If
        'If angle >= 270 And angle < 360 Then
        '    MsgBox("270-360.angulo= " & angulo & " centroide(x)= " & centroide(0) & " centroide(y)= " & centroide(1) & " x= " & x & "y= " & y)
        '    p(0) = centroide(0) + 2 * x
        '    p(1) = centroide(1) - 2 * y
                ...
                For Each element In conjunto
                    ...
                    If element.handle <> perimetro.Handle Then
                        If element.ObjectName <> "AcDb3dSolid" Then ' se exluye el auto 
                            ...
                            If element.ObjectName = "AcDbCircle" Then
                                'respuesta = GetDictionaryVehiculo(semaforo, "ANGULOACTUAL", valor)
                            End If
