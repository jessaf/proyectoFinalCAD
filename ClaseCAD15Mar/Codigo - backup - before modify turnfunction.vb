Module Codigo
    ...
    Public Function seguimientoCalle()
        ...
        CreandoVehiculos(conjuntoCalles, conjuntoVehiculos, contorno, lista)

        '===================================================================================
        'PASO 3 Girar los vehiculos segun la pendiente de la calle 
        '===================================================================================

        GirarVehiculosSegunOrientacionDeLaCalle(conjuntoCalles, conjuntoVehiculos, numCalles, lista)

        ''===================================================================================
        ''PASO 4 Mover los vehiculos a travez de una porcion de la calle
        ''===================================================================================

        moverVehiculo(conjuntoCalles, conjuntoVehiculos, 200)
        ...
        'MsgBox("Se han encontrado" & contadorCalles & "calles")
        'conjunto.Delete()
    End Function
    ...
    Public Function CreandoVehiculos(conjuntoCalles As AcadSelectionSet, conjuntoVehiculos As AcadSelectionSet, contorno As AcadEntity, lista As ListBox)
        ...
        For Each calle In conjuntoCalles
            ...
            p1 = calle.startpoint
            p2 = calle.endpoint
            'lista.Items.Add(">>>>>Longitud = " & calle.length)
            'lista.Items.Add(">>>>>Coordenada inciial = " & p1(0) & "," & p1(1) & "," & p1(2))
            'lista.Items.Add(">>>>>Coordenada final = " & p2(0) & "," & p2(1) & "," & p2(2))
            ...
            If (UBound(arrcontorno) > 0) Then
                ...
                ''-----------------------------------------------------------------------------------------------------------
                ''Esta porcion de codigo sirve para imprimir las coordenadas del punto de interseccion
                'str = Nothing
                'For I = LBound(arrcontorno) To UBound(arrcontorno)
                '    str = str & "//" & arrcontorno(I)
                'Next I
                ''Este if es redundante pero por alguna razon 
                'If Not IsNothing(str) Then
                '    lista.Items.Add("posicion interseccion= " & str & "   startpointline= " & p1(0) & "," & p1(1) & "//" & p1(2) & "   endpointline= " & p2(0) & "//" & p2(1) & "//" & p2(2) & "   handeler= " & calle.Handle)
                'End If
                ''-----------------------------------------------------------------------------------------------------------
            End If
    ...
    Public Function GirarVehiculosSegunOrientacionDeLaCalle(conjuntoCalles As AcadSelectionSet, conjuntoVehiculos As AcadSelectionSet, numCalles As Integer, lista As ListBox)
        ...
        Dim anguloactual As Double
        'Dim arrIntersectionVehiculoCalle() As Double
        Dim centroideVehiculo() As Double = Nothing
        Dim callesdeinterseccion As Collection
        ...
        'lista.Items.Add("=======================") '
        For Each calle In conjuntoCalles
            'Obtenemos los puntos de la calle
            p1 = calle.startpoint
            p2 = calle.endpoint
            'callesdeinterseccion = AnalizarInterseccion(conjuntoCalles, calle, numCalles)
            For Each vehiculo In conjuntoVehiculos
                ...
                If valor = "VACIO" Then
                    anguloactual = 0.0
                    'MsgBox("El angulo esta vacio")
                Else
                    anguloactual = CDbl(valor)
                    'MsgBox("El angulo no esta vacio = " & anguloactual.ToString)
                End If

                'Se supone que lo siguiente deberia funcionar pero esta lanzando excepciones
                'arrIntersectionVehiculoCalle = calle.IntersectWith(prueba, AcExtendOption.acExtendNone)
                'Asi que se procede de la siguiente manera
                'lista.Items.Add("Entra a calle/vehiculo") '
                centroideVehiculo = vehiculo.centroid
                'lista.Items.Add("---------------------------------------------------------------")
                'lista.Items.Add("vHandeler=" & vehiculo.Handle & "    cHandeler=" & calle.Handle)
                'lista.Items.Add("Centroid= " & centroideVehiculo(0) & "/" & centroideVehiculo(1))
                'lista.Items.Add("p1x= " & p1(0) & ",p1y= " & p1(1) & "p2x= " & p2(0) & ",p2y= " & p2(1))
                'lista.Items.Add("vHandeler=" & vehiculo.Handle & "    cHandeler=" & calle.Handle & "    Centroid= " & centroideVehiculo(0) & "/" & centroideVehiculo(1))
                If (centroideVehiculo(0) = p1(0) And centroideVehiculo(1) = p1(1)) Then
                    'lista.Items.Add("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ match found")
                    'calculamos la pendeinte de la linea
                    ...
                    'Primero le quito el angulo que tenia anteriormente para luego aplicarle el nuevo segun la pendiente de la siguiente recta 
                    If anguloactual > 0 Then
                        vehiculo.Rotate(p1, anguloactual * -1)
                    End If
                    If anguloactual < 0 Then
                        vehiculo.Rotate(p1, anguloactual * -1)
                    End If
                    ...
                    'Dependiendo de si el angulo es positivo o negativo giramos el vehiculo
                    If (pendiente < 0 And pendiente <> 0) Then

                        vehiculo.Rotate(p1, angle * -1)
                        'MsgBox("pendiente negativa")
                        'Actualizo el valor del angulo en el diccionario del vehiculo
                        UpdateDictionaryToVehiculo(vehiculo, angle * -1)
                    End If
                    If (pendiente > 0 And pendiente <> 0) Then
                        vehiculo.Rotate(p1, angle)
                        'MsgBox("pendiente positiva")
                        'Actualizo el valor del angulo en el diccionario del vehiculo
    ...
    Public Function moverVehiculo(conjuntoCalles As AcadSelectionSet, conjuntoVehiculos As AcadSelectionSet, numMov As Integer)
        ...
        For Each calle In conjuntoCalles
            ...
            For Each vehiculo In conjuntoVehiculos
                ...
                centroideVehiculo = vehiculo.centroid
                'El or esta por si los vehiculos aparecen en la parte derecha del contorno (esta pendiente agregar esto a AgregarVehiculos())
                If ((centroideVehiculo(0) = p1(0) And centroideVehiculo(1) = p1(1)) Or (centroideVehiculo(0) = p2(0) And centroideVehiculo(1) = p2(1))) Then
                    'MsgBox("Se encontro coincidencia")
                    ...
                    'lista.Items.Add(">>>>>p2(x)= " & p2(0) & ", p2(y)= " & p2(1))
                    'lista.Items.Add(">>>>>p1(x)= " & p1(0) & ", p1(y)= " & p1(1))
                    'lista.Items.Add(">>>>>resta x= " & p2(0) - p1(0) & ", resta y= " & p2(1) - p1(1))
                    'lista.Items.Add(">>>>>incX= " & (p2(0) - p1(0)) / numMov & ", incY= " & (p2(1) - p1(1)) / numMov)
                    ...
                    For count = 1 To numMov
                        ...
                        'lista.Items.Add(">>>>>currentposition x = " & currentPosition(0) & ", currentposition y= " & currentPosition(1))
                        'lista.Items.Add(">>>>>next x = " & nextPosition(0) & ", netx y= " & nextPosition(1))
